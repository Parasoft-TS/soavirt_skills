...
- `docs/skills/client-tools/skill-020-rest-client-none-mode-workflow.md`
  - **State:** Validated
  - **Notes:** unconstrained REST Client creation/configuration is the current recommended path, including validated read-merge-write Basic-auth updates on existing clients through the native `httpAuthentication` subtree with readback confirmation of the persisted auth subtree, caller-owned concrete-host normalization of generated template clients to `${BASEURL}` plus relative path/query, and validated primitive-leaf request-body parameterization using `${TOKEN}` values sourced from either true datasource columns or tool-produced custom columns. Complex-object or whole-subtree substitution remains out of scope.
- `docs/skills/client-tools/skill-034-soap-client-http-lifecycle.md`
  - **State:** Defined
  - **Notes:** first-pass SOAP Client lifecycle card covering the API-exposed HTTP request/transport/misc surface; it now also documents the same primitive-leaf `${TOKEN}` request-parameterization rule used for downstream datasource/custom-column bindings inside request XML, while full runtime validation of that branch, full WSDL-tab parity, and non-HTTP transport authoring remain pending.
- `docs/skills/client-tools/skill-059-constrained-rest-client-yaml-fallback.md`
  - **State:** Validated
  - **Notes:** validated v1 owner for single constrained REST Client lifecycle work; reviewed in wave 2 and clarified as a hybrid card with local `.tst`/YAML authority for asset structure and API-only entry for runtime ids, tool lifecycle routes, constrained JSON body normalization, constrained-client Basic-auth updates through the native REST Client auth subtree, and inherited primitive-leaf `${TOKEN}` body parameterization for datasource/custom-column-backed values. Future OAuth2/YAML-auth editing and complex-object/subtree substitution remain out of scope.
...
- `docs/skills/data-exchange/skill-075-data-generator-lifecycle.md`
  - **State:** Defined
  - **Notes:** full CRUD lifecycle card for Data Generator Tools defined from the local OpenAPI schema plus project-backed `.tst` evidence. The card now documents the overloaded "data source column" terminology, the validated create/reorder/custom-column downstream-consumption pattern from the Parasoft Demo App category-name flow, the API-vs-YAML serialization split for custom-column bindings, and the fact that runtime execution is optional unless the caller or owning workflow asks for it. Broader CRUD/runtime coverage is still incomplete, so the card remains `Defined` rather than `Validated`.
