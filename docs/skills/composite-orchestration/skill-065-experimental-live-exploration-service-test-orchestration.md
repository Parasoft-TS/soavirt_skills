...
30a. When active project context already contains the canonical bootstrap external environment file for the selected semantic environment, instruct Skill 022 to use its `createEnvironment` reference-external branch against that exact file during generation.
30b. If that canonical bootstrap environment file is not resolved confidently for this branch, fall back to Skill 022's normal default generation path rather than guessing an external environment location.
...
33. Read back the generated root suite, child structure, referenced-environment state when present, and any generated/local environment context.
34. When a bootstrap external environment file was attached during generation, inspect that file and the generated template suite together before choosing any replacement variable names:
   - read the external environment file to discover the actual OpenAPI/base-url variable names present for the selected semantic environment, for example prefixed names such as `PARABANK_SWAGGER` / `PARABANK_BASEURL`
   - inspect the generated REST Clients and schema references to see whether Skill 022 already parameterized them with those discovered names
   - do not assume generic `${OPENAPI}` or `${BASEURL}` names are always correct
35. Evidence-backed rule for this lane:
   - Skill 022 does not resolve `location.url=${...}` from the attached external environment file during generation; source location must already be concrete/resolved before the create call
   - but when the create call uses a concrete OpenAPI URL and the attached external environment file contains matching OpenAPI/base-url variables, Skill 022 may emit those external variable names directly into the generated `.tst`
36. Immediately after create/readback, branch template normalization by what the readback proves:
   - if generated REST Clients and schema references are already parameterized to the correct external-environment variable names, preserve that parameterization and do not overwrite it with an invented generic `${BASEURL}`
   - if generation attached the external environment structurally but still left concrete URLs, use Skill 064 to ensure the caller-owned local environment context needed for template normalization exists, then add or update the correct variable name derived from the external environment file or other confirmed branch context
   - do not guess when no single correct base-url variable name is confirmed
37. Before the generated suite is frozen as source-of-copy, normalize only the still-concrete generated template-suite REST Clients through Skill 020 so URLs rooted at that confirmed base become the proved-correct variable reference plus the preserved relative path and query.
38. Preserve already-parameterized, relative, or non-matching resource templates as-is, and stop or defer rather than guessing when a generated URL cannot be safely decomposed into confirmed base plus relative remainder.
39. Disable the generated OpenAPI-derived suite through Skill 074 and preserve it as the template/source-of-copy area only after the variable-name inspection and any needed normalization step are complete; do not mutate it into executable authored coverage.
40. Create or reuse the sibling top-level suite `AI Generated Tests` through Skill 009.
...
46. Configure each copied REST Client through Skill 020 using the ledger-approved method, endpoint, auth, payload, expected-code, and parameterization decisions, while preserving whatever inherited endpoint parameterization the generated template readback proved correct unless the approved branch explicitly needs a different base strategy.
